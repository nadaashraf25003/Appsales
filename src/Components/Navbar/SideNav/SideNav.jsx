...
// This map allows your userData.ts strings to still work
const iconMap = {
  Dashboard, AccountCircle, Analytics, Notifications, PointOfSale,
  ShoppingCart, AssignmentReturn, People, Inventory, Category,
  LocalShipping, AccountBalance, Receipt, Assessment, BarChart,
  Settings, Paid, Group, Apartment
};
        ...
        <div className="flex items-center gap-3">
          ...
          {sidebarOpen && (
            <div className="overflow-hidden">
              <p className="font-semibold text-sm dark:text-white truncate">{user.userTopNav.name}</p>
              <p className="text-xs text-gray-500 dark:text-gray-300 truncate">{user.userTopNav.role}</p>
            </div>
          )}
        </div>
      </div>
              ...
              <>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {sidebarOpen && <span className="uppercase text-xs font-semibold">{group.section}</span>}
                  </div>
                  {sidebarOpen && (openSections[group.section] ? <ExpandLess /> : <ExpandMore />)}
                </button>
